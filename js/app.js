...
const App = (function () {
  ...
  function navigate(screen, opts) {
    opts = opts || {};
    if (!views[screen]) {
      console.warn('[App] Tela inválida:', screen);
      return;
    }
    if (
      !opts.force &&
      ...
      showToast(QUIZ_LOCK_MESSAGE, 'warning');
      return;
    }
    if (
      typeof QuizView !== 'undefined' &&
      typeof QuizView.clearTimers === 'function'
    ) {
  ...
  function finishQuiz() {
    ...
    Store.addSession({
      ...
      topic: resolveTopicLabel(s),
      correct: correct,
      total: total,
      pct: pct,
      durationSec: Math.round((Date.now() - s.sessionStart) / 1000),
      timedOutCount: s.answers.filter(function (a) {
        return a.isTimeout;
      }).length,
    });
