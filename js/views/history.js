const HistoryView = (function () {
  ...
  function render(el) {
    ...
    var subjectsHTML = opts.subjects.map(function(e) { return '<option value="' + e[0] + '">' + e[1] + '</option>'; }).join('');
    var topicsHTML   = opts.topics.map(function(e) { return '<option value="' + e[0] + '">' + e[1] + '</option>'; }).join('');
    ...
    el.innerHTML =
      '<div class="card">' +
        ...
        '<div class="history-filters">' +
          ...
          '<select id="filter-subject" class="history-select" aria-label="Matéria"><option value="">Todas as matérias</option>' + subjectsHTML + '</select>' +
          '<select id="filter-topic"   class="history-select" aria-label="Assunto"><option value="">Todos os assuntos</option>' + topicsHTML   + '</select>' +
    ...
    function applyFilters() {
      ...
      var filtered = history.filter(function(item) {
        if (subject && (item.subject || extractSubjectLabel(item)) !== subject) return false;
        if (topic   && (item.topicId  || item.topic) !== topic)                return false;
        if (q) {
          var hay = normalize(extractTopicLabel(item) + ' ' + extractSubjectLabel(item) + ' ' + (item.date || ''));
          if (!hay.includes(q)) return false;
        }
        return true;
      });

      if (sort === 'pct-desc') filtered = filtered.slice().sort(function(a,b){ return b.pct - a.pct; });
      if (sort === 'pct-asc')  filtered = filtered.slice().sort(function(a,b){ return a.pct - b.pct; });
    ...
    function renderStats(filtered) {
      ...
      if (!filtered.length) {
        statsEl.innerHTML = '';
        summaryEl.textContent = 'Nenhuma sessão encontrada com os filtros aplicados.';
        return;
      }
      var avg     = Math.round(filtered.reduce(function(s,i){ return s+i.pct; }, 0) / filtered.length);
      var best    = Math.max.apply(null, filtered.map(function(i){ return i.pct; }));
      var totalQ  = filtered.reduce(function(s,i){ return s+(i.total||0); }, 0);
      var correct = filtered.reduce(function(s,i){ return s+(i.correct||0); }, 0);
      summaryEl.textContent = filtered.length + ' sessão(ões) encontrada(s)';
      statsEl.innerHTML =
        '<div class="stat-card"><div class="stat-val">' + filtered.length + '</div><div class="stat-lbl">Sessões</div></div>' +
        '<div class="stat-card"><div class="stat-val">' + avg + '%</div><div class="stat-lbl">Média</div></div>' +
        '<div class="stat-card"><div class="stat-val">' + best + '%</div><div class="stat-lbl">Melhor nota</div></div>' +
        '<div class="stat-card"><div class="stat-val">' + correct + '/' + totalQ + '</div><div class="stat-lbl">Acertos totais</div></div>';
    }
    ...
    function renderChart(filtered) {
      ...
      chartInstance = new Chart(canvas, {
        type: 'line',
        data: {
          labels: last15.map(function(_,i){ return '#'+(i+1); }),
          datasets: [{
            label: 'Nota (%)',
            data: last15.map(function(i){ return i.pct; }),
            borderColor: '#4F46E5',
            backgroundColor: 'rgba(79,70,229,0.08)',
            tension: 0.4, fill: true,
            pointBackgroundColor: last15.map(function(i){ return getGrade(i.pct).cls === 'grade-a' ? '#10B981' : '#4F46E5'; })
          }]
        },
        options: {
    ...
    function renderList(filtered) {
      ...
      if (!filtered.length) {
        listEl.innerHTML = '<div class="history-empty-filter">Nenhuma sessão corresponde aos filtros.</div>';
        return;
      }
      listEl.innerHTML = filtered.map(function(item) {
        var g = getGrade(item.pct);
        return '<div class="session-card" role="article">' +
          '<div class="session-badge ' + g.cls + '">' + item.pct + '%</div>' +
          '<div class="session-info">' +
            '<div class="session-date">' + (item.date||'') + ' · ' + extractSubjectLabel(item) + '</div>' +
            '<div class="session-topic">' + extractTopicLabel(item) + '</div>' +
            '<div class="session-meta">' + item.correct + '/' + item.total + ' corretas · ' +
              durationLabel(item.durationSec) +
              (item.timedOutCount ? ' · ⏰ ' + item.timedOutCount + ' timeout' : '') +
            '</div>' +
          '</div>' +
          '<div class="session-side">' +
            '<div class="session-stars">' + g.stars + '</div>' +
            '<div class="session-level">' + g.label + '</div>' +
          '</div>' +
        '</div>';
      }).join('');
    }
